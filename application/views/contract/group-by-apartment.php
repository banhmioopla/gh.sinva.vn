...
<table class="table-contract  table table-dark table-bordered">
    ...
    <?php foreach ($list_apartment as $apm): ?>
        <?php
        $apm_list_contract = $this->ghContract->get([
            'status <>' => 'Cancel',
        ...
        <tr class="border border-warning mt-2">
            <td colspan="3"><h4><?= $apm["address_street"] ?></h4> </td>
            <td colspan="10"> <?= count($apm_list_contract) ?> hợp đồng </td>
        </tr>
        <?php foreach($apm_list_contract as $row ): ?>
            <?php
